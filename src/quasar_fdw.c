...
static void quasarAddForeignUpdateTargets(Query *parsetree,
                                                 RangeTblEntry *target_rte,
                                                 Relation target_relation);

static List *quasarPlanForeignModify(PlannerInfo *root,
                                       ModifyTable *plan,
                                       Index resultRelation,
                                       int subplan_index);

static void quasarBeginForeignModify(ModifyTableState *mtstate,
                                     ResultRelInfo *rinfo,
                                     List *fdw_private,
                                     int subplan_index,
                                     int eflags);

static TupleTableSlot *quasarExecForeignInsert(EState *estate,
                                               ResultRelInfo *rinfo,
                                               TupleTableSlot *slot,
                                               TupleTableSlot *planSlot);

static TupleTableSlot *quasarExecForeignUpdate(EState *estate,
                                               ResultRelInfo *rinfo,
                                               TupleTableSlot *slot,
                                               TupleTableSlot *planSlot);

static TupleTableSlot *quasarExecForeignDelete(EState *estate,
                                               ResultRelInfo *rinfo,
                                               TupleTableSlot *slot,
                                               TupleTableSlot *planSlot);

static void quasarEndForeignModify(EState *estate,
                                   ResultRelInfo *rinfo);

static int  quasarIsForeignRelUpdatable(Relation rel);

#endif

static void quasarExplainForeignScan(ForeignScanState *node,
                                     struct ExplainState * es);

#if (PG_VERSION_NUM >= 90300)
static void quasarExplainForeignModify(ModifyTableState *mtstate,
                                       ResultRelInfo *rinfo,
                                       List *fdw_private,
                                       int subplan_index,
                                       struct ExplainState * es);
...
#if (PG_VERSION_NUM >= 90200)
static bool quasarAnalyzeForeignTable(Relation relation,
...
static void quasarGetForeignJoinPaths(PlannerInfo *root,
                                      RelOptInfo *joinrel,
                                      RelOptInfo *outerrel,
                                      RelOptInfo *innerrel,
                                      JoinType jointype,
...
static RowMarkType quasarGetForeignRowMarkType(RangeTblEntry *rte,
                                               LockClauseStrength strength);

static HeapTuple quasarRefetchForeignRow(EState *estate,
                                         ExecRowMark *erm,
                                         Datum rowid,
                                         bool *updated);

static List *quasarImportForeignSchema(ImportForeignSchemaStmt *stmt,
                                       Oid serverOid);
 ...
 *
 * These next two are not actually used by quasar, but something like this
...
#if (PG_VERSION_NUM >= 90300)
      fdwroutine->IsForeignRelUpdatable = quasarIsForeignRelUpdatable;
      fdwroutine->AddForeignUpdateTargets = quasarAddForeignUpdateTargets;          /* U D */
      fdwroutine->PlanForeignModify = quasarPlanForeignModify; /* I U D */
      fdwroutine->BeginForeignModify = quasarBeginForeignModify;        /* I U D */
      fdwroutine->ExecForeignInsert = quasarExecForeignInsert; /* I */
      fdwroutine->ExecForeignUpdate = quasarExecForeignUpdate; /* U */
      fdwroutine->ExecForeignDelete = quasarExecForeignDelete; /* D */
      fdwroutine->EndForeignModify = quasarEndForeignModify;      /* I U D */
#endif

      /* support for EXPLAIN */
      fdwroutine->ExplainForeignScan = quasarExplainForeignScan;        /* EXPLAIN S U D */
#if (PG_VERSION_NUM >= 90300)
      fdwroutine->ExplainForeignModify = quasarExplainForeignModify;    /* EXPLAIN I U D */
#endif

#if (PG_VERSION_NUM >= 90200)
      /* support for ANALYSE */
      fdwroutine->AnalyzeForeignTable = quasarAnalyzeForeignTable;            /* ANALYZE only */
#endif


#if (PG_VERSION_NUM >= 90500)
      /* Support functions for IMPORT FOREIGN SCHEMA */
      fdwroutine->ImportForeignSchema = quasarImportForeignSchema;

      /* Support for scanning foreign joins */
      fdwroutine->GetForeignJoinPaths = quasarGetForeignJoinPaths;

      /* Support for locking foreign rows */
      fdwroutine->GetForeignRowMarkType = quasarGetForeignRowMarkType;
      fdwroutine->RefetchForeignRow = quasarRefetchForeignRow;
...
#if (PG_VERSION_NUM >= 90300)
static void
quasarAddForeignUpdateTargets(Query *parsetree,
                              RangeTblEntry *target_rte,
                              Relation target_relation)
{
      /*
...
static List *
quasarPlanForeignModify(PlannerInfo *root,
                        ModifyTable *plan,
                        Index resultRelation,
                        int subplan_index)
{
...
static void
quasarBeginForeignModify(ModifyTableState *mtstate,
                         ResultRelInfo *rinfo,
                         List *fdw_private,
                         int subplan_index,
                         int eflags)
...
static TupleTableSlot *
quasarExecForeignInsert(EState *estate,
                        ResultRelInfo *rinfo,
                        TupleTableSlot *slot,
                        TupleTableSlot *planSlot)
{
...
static TupleTableSlot *
quasarExecForeignUpdate(EState *estate,
                        ResultRelInfo *rinfo,
                        TupleTableSlot *slot,
                        TupleTableSlot *planSlot)
{
...
static TupleTableSlot *
quasarExecForeignDelete(EState *estate,
                        ResultRelInfo *rinfo,
                        TupleTableSlot *slot,
                        TupleTableSlot *planSlot)
{
...
static void
quasarEndForeignModify(EState *estate,
                       ResultRelInfo *rinfo)
{
      /*
...
static int
quasarIsForeignRelUpdatable(Relation rel)
{
      /*
...
static void
quasarExplainForeignScan(ForeignScanState *node,
                         struct ExplainState * es)
{
      /*
...
#if (PG_VERSION_NUM >= 90300)
static void
quasarExplainForeignModify(ModifyTableState *mtstate,
                           ResultRelInfo *rinfo,
                           List *fdw_private,
                           int subplan_index,
                           struct ExplainState * es)
...
#if (PG_VERSION_NUM >= 90200)
static bool
quasarAnalyzeForeignTable(Relation relation,
                          AcquireSampleRowsFunc *func,
                          BlockNumber *totalpages)
{
      /* ----
...
#if (PG_VERSION_NUM >= 90500)
static void
quasarGetForeignJoinPaths(PlannerInfo *root,
                          RelOptInfo *joinrel,
                          RelOptInfo *outerrel,
                          RelOptInfo *innerrel,
                          JoinType jointype,
...
static RowMarkType
quasarGetForeignRowMarkType(RangeTblEntry *rte,
                            LockClauseStrength strength)
{
      /*
...
static HeapTuple
quasarRefetchForeignRow(EState *estate,
                        ExecRowMark *erm,
                        Datum rowid,
                        bool *updated)
{
...
static List *
quasarImportForeignSchema(ImportForeignSchemaStmt *stmt,
                          Oid serverOid)
{
      /*
